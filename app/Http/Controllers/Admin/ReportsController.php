...
class ReportsController extends Controller
{
    ...
    public function details()
    {
        ...
        $resp['types'] = Utils\StoreUtils::storeTypes();
    ...
    {   
        $data['locations'] = Utils\Common::getAllLocations();
        $data['cities'] = Utils\Common::getAllCities();
        $data['stores'] = Utils\Common::getAllStores();
        $data['states'] = Utils\Common::getAllStates();
        //  $data['cities'] = [];
        // $data['dealers'] = [];
        return response()->json($data);
    }
    ...
    public function getStoresByState($state)
    {
        $cities = Store::whereHas('activeCities', function ($query) use ($state) {
            $query->has('activeStores')
                ->where('state_id', '=', $state);
        })->pluck('store_name', 'id');

        return response()->json($cities);
    }
    public function getLocationByState($state)
    {
        $cities = Store::whereHas('activeCities', function ($query) use ($state) {
            $query->has('activeStores')
                ->where('state_id', '=', $state);
    ...
    {
        $stores = Store::where('city_id', '=', $city)
            ->pluck('store_name', 'id');
        ...
        if(!empty($data)) {
            foreach ($data as $value) {
                ...
                if(in_array($month,$groupedData['month'])) {
                    ...
                    $groupedData['data'][$month]['missed']+=$value['missed'];
                    $groupedData['data'][$month]['ivrdrop']+=$value['ivrdrop'];
                    $groupedData['data'][$month]['offline']+=$value['offline'];
    ...
    public function getFilterClusters(){
        $params = Input::all();
        $data['clusters'] = Utils\ClusterUtils::getFilterClusters($params['region']);
        return response()->json($data);
    }
    public function getFilterStores(){
        $params = Input::all();
        $data['stores'] = Utils\StoreUtils::getFilterStores($params['region'],$params['cluster']);
        return response()->json($data);
    }
